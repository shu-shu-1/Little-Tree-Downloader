...
file descriptor with seek+write pairs serialized by a lock, and preallocates the
target file so concurrent chunk writes to any offset are safe. Downloaded data
lands in a `.part` file that is atomically renamed to the final name on success.
...
**Note**: `FileWriter` is used internally by the downloader; no manual
